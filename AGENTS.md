...
The short version: any change under `plugins/<name>/` must bump that plugin's version via `node scripts/bump-plugin-version.mjs <name> <minor|patch>`, because plugins install to a version-keyed path and an unbumped change never reaches an installed user. This applies equally to all three manifests — `plugins/<name>/plugin.json` (the Agent Plugins spec location), `.claude-plugin/plugin.json`, and `.codex-plugin/plugin.json` — which must always agree, which is why the bump script writes all of them. A pre-commit hook enforces this and prints the command you need. The document covers which digit to choose, which is the part the hook cannot decide.
