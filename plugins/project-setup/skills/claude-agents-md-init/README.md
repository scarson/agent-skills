...
- **Target-conditional emission** (a leaner CLAUDE.md and a fuller AGENTS.md from the same template) — deferred, not declined. The assessment called it the most actionable structural idea, and the skill's substitution machinery could support it. It was not taken now because its premise is weaker than the assessment assumed: this marketplace ships `.codex-plugin` manifests, so the superpowers skills route on AGENTS.md frameworks too, and the two-variant maintenance cost (every future edit reasoning about both variants, doubled sibling-sync surface) buys mostly the §Learning and Memory delta. Revisit if the capability gap between the sibling frameworks widens.
