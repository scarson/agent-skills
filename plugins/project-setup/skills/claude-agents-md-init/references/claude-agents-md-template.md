...
- **Run the pre-PR gate before opening any PR.** Rebase onto fresh `origin/[PRIMARY BRANCH]`, verify the *rebased* tree, and re-check ancestry before pushing — CI builds the merge result, not your branch tip, so a stale base means CI tests a tree nobody ran. Full steps: `docs/git-strategy.md` §Pre-PR gate.
...
- **If you are unsure whether an existing entry covers your finding, you have not read it closely enough.** Read it, then decide — and say which entries you considered when you report the work. The same discipline applies when the search returns *no* candidates: an empty result is a finding to verify, not a green light to add — widen to the failure's symptom vocabulary and scan the relevant section's entries directly before concluding nothing owns it.
