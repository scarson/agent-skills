...
- Pin by immutable identity: commit SHA for VCS, `name@version` **plus integrity hash** for registries, image **digest** (not a mutable tag) for containers. For **NuGet**, pin exact `<PackageReference Version="…">` (no floating `*` or open ranges) and commit a `packages.lock.json` — PackageReference only, since `packages.config` is itself a flat pinned list.
- Disable install scripts by default (`--ignore-scripts` and equivalents); run first execution in a sandbox. For **NuGet**, restore in **locked mode** (`RestorePackagesWithLockFile` + `--locked-mode` / `RestoreLockedMode`) so a changed graph fails the build instead of silently resolving — but not alongside per-RID publishing (`dotnet publish -r <rid>`): the lock file records an exact runtime-identifier set, so each RID-specific restore is a mismatch (`NU1004`), and declaring every RID up front fails too, because passing one narrows the set against the file's many.
- Fail the build on known advisories instead of printing them. For **NuGet**, auditing already runs during restore, but only warns — raise `NU1901`–`NU1904` through `WarningsAsErrors` in `Directory.Build.props`. Two limits decide whether that works: `NuGetAuditMode` defaults to `direct` below `net10.0`, so set `all` for transitive coverage; and on `packages.config` the gate cannot be built at all, because MSBuild severity properties are unsupported there and audit itself needs VS 17.10+ rather than the dotnet CLI. To accept one finding, prefer a `NuGetAuditSuppress` item naming the advisory URL over an exemption that blinds the gate to everything else.
- Check what verification you already have before configuring more. For **NuGet**, signed-package verification runs during restore with no configuration (always on Windows, by default since the .NET 8 SDK on Linux, and deliberately off on macOS). `signatureValidationMode=require` adds certificate allow-listing on top and pays off only where every build leg is Windows — `dotnet restore` lists `trustedSigners` among the settings it ignores, so on a mixed or Linux pipeline it reads as enforcement while doing nothing.
