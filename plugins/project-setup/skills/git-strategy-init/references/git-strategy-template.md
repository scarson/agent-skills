...
Out of scope: most release-cut mechanisms — `release/*` branches, hotfix branches cut from the release branch, and environment branches like `staging` and `production`. The two-branch-gitflow case (the integration-branch → release-branch publication PR) IS covered: see the `## Release branch` section near the end of this doc. `git-strategy-init` retains that section when it detects two-branch gitflow and removes it otherwise. For all other release-cut shapes, the invariants below still apply to feature work converging on the integration branch — you'll need separate policy for whatever ships *off* that branch.
...
3. Handle the `## Release branch` section near the end of the doc:
    - **If your project uses two-branch gitflow** (a separate release branch — e.g. `dev`/`develop` integration with `main`/`master` release-only): substitute `[RELEASE_BRANCH]` → your release branch name throughout that section. The `main` mentions inside the section are integration-branch references and are already handled by step 2.
    - **Otherwise** (GitHub flow / trunk-based / any single-branch project): delete the entire `## Release branch` section AND its `[Release branch](#release-branch)` line in the §Contents list above.
4. Review the command blocks after replacement to confirm they still make sense.
...
- [Release branch](#release-branch) — publication PR mechanics (two-branch gitflow only; remove this line if your project has no separate release branch)
...
This project uses **two-branch gitflow**: `main` is the integration branch (everything above this section), and `[RELEASE_BRANCH]` is the release branch (everything below). This section governs the publication mechanism between them.

**Role of `[RELEASE_BRANCH]`.** `[RELEASE_BRANCH]` represents the published state of the project. The commit log on `[RELEASE_BRANCH]` is the project's public history — readable by anyone visiting the repo. It is updated only via deliberate `main` → `[RELEASE_BRANCH]` publication PRs, never by ad-hoc commits.

**Invariants for `[RELEASE_BRANCH]`:**

1. **No direct commits to `[RELEASE_BRANCH]`.** Not from worktrees, not from the root checkout, not via emergency reset. The single update mechanism is the `main` → `[RELEASE_BRANCH]` PR described below.
2. **No feature branches target `[RELEASE_BRANCH]`.** Feature branches target `main`. Promotion to `[RELEASE_BRANCH]` happens in batches via the publication PR.
3. **`[RELEASE_BRANCH]` advances strictly forward** — fast-forward or merge commit. Never rewritten, never force-pushed.

**Publication PR mechanics:**

```bash
# From the root checkout (which stays on main — Invariant 1):
git fetch origin main
git fetch origin [RELEASE_BRANCH]
git log --oneline origin/[RELEASE_BRANCH]..origin/main   # what will be published

# Open the publication PR:
gh pr create --base [RELEASE_BRANCH] --head main \
...
#   --delete-branch=false: main is the persistent integration branch,
#   NOT ephemeral. The main → [RELEASE_BRANCH] PR merges integration
#   history into release but does NOT delete main — work continues on
#   main for the next publication cycle.
```
...
**`[RELEASE_BRANCH]` workflow rules for daily work:**

- Never `git checkout [RELEASE_BRANCH]` in the root checkout. Read `[RELEASE_BRANCH]`'s state via `git log origin/[RELEASE_BRANCH]`, `git show origin/[RELEASE_BRANCH]:<path>`, or a short-lived read-only worktree (same pattern as §Living documents).
- Never run `git reset --hard origin/[RELEASE_BRANCH]` against local `[RELEASE_BRANCH]` while local `[RELEASE_BRANCH]` has divergent commits — that would destroy commits the publication PR would have moved. If local `[RELEASE_BRANCH]` has drifted from `origin/[RELEASE_BRANCH]`, that's a red flag (Invariant 1 above was violated); investigate before resetting.
- The §Recovery from a messy state procedure applies to `[RELEASE_BRANCH]` analogously: any local-`[RELEASE_BRANCH]`-ahead-of-`origin/[RELEASE_BRANCH]` divergence is anomalous and requires user-approved investigation.
