...
0. **Search for a check item that already owns the mechanism** — grep this document for the mechanism, not the surface, and read candidates in full. An empty result means widen (grep the symptom vocabulary; scan the likely topic section's items directly) before concluding nothing owns it. If a check item here owns it, strengthen that item (add the 🔥 note or the missing case) instead of adding a new one. Also grep `docs/pitfalls/implementation-pitfalls.md`: an implementation entry owning the mechanism does NOT replace a check item here — the two documents play different roles — but it changes your step 4 below from "check for" to "add" the cross-reference, and its update procedure may owe the new case too.
