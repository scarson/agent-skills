...
Will run in sequence:
  1. claude-agents-md-init     — installs CLAUDE.md and AGENTS.md at the project
                          root from a single model-tuned template (RFC 2119
...
- **Running the wrapper without reading the sub-skills' SKILL.md files.** If your framework doesn't have a native skill-invocation primitive, you MUST read and follow each sub-skill's SKILL.md in full — don't skip the pre-flight checks or auto-detect steps. Those steps prevent data loss (detecting existing files before overwriting). The same applies at the other end of each sub-skill: every one of them backs up the files it edits and runs a content-preservation gate against that backup before reporting. Inlining a sub-skill's happy path while dropping its backup-and-compare steps is how a wrapper run silently deletes project content that the sub-skill, run directly, would have caught and restored.
...
- **`claude-agents-md-init`** (sibling sub-skill): installs `CLAUDE.md` and/or `AGENTS.md` at the project root from a single model-tuned template (universal ruleset + placeholder blocks for project-specific content; per-target substitutions for intro line and sibling reference).
