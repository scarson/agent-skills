...
Handle its four terminal states: **completed** → proceed to Step 3. **Abort to brainstorming** (the gate concluded the design solves the wrong problem) → return to Step 1. **Terminated-for-execution** (the review's convergence circuit breaker fired — the design carries verify-first annotations at its open spans) → proceed to Step 3, instructing the plan writer that every annotated span is verify-before-build: the plan's tasks that touch a flagged span carry its verification as an explicit step, and the design is treated as partially verified, not certified. **Stopped awaiting user** → do not proceed; the review's ledger holds the posed questions. (A Phase 0 readiness-gate stop — the artifact is not yet a reviewable design — is handled like abort: return to Step 1 and finish the design.)
...
- **Bolting extra ad-hoc review loops onto `design-review-cycle`'s output** — the cycle already folds fixes into the design and terminates on a clean two-leg closure. Its terminal state is binding: completed → Step 3; abort → Step 1; terminated-for-execution → Step 3 with the flags carried into the plan. Improvised additional rounds after a clean closure are the manufacture failure its success definition names — and improvised extra rounds after the circuit breaker fired are exactly the churn it exists to stop.
