...
**Clean:** a leg is clean when it returns the closure ref it was handed and no finding that dispositions as confirmed-substantive — and every finding it returned, including demotions, has its ledger row written first. A dirty leg is not special: findings → Phase 3 → supplementary gate if needed → Phase 5 diff check → closure re-runs. **The cycle ends only when both legs pass against the same snapshot**; each checklist line records that SHA. **Degraded mode:** if dispatch is genuinely unavailable, the runner MAY perform both audits itself and MUST disclose that neither leg met the cold/independent requirement; it MUST NOT label a self-review "cold."
...
Three terminal states, reported to the user and to any invoking skill: **completed** (both legs clean); **abort to brainstorming** (a gate concluded wrong-problem — recorded without shame; the design goes back to design work, not onward to planning); **stopped awaiting user** (gate posed, user unreachable — run header holds the posed gate; `handoff` produces the artifact). **Whichever terminal fires, writing it to the run header's Terminal state line is the last ledger edit, and that update rides the terminal's single closing commit** — for completed, the automatic completion commit this section mandates; for abort and stopped-awaiting-user, an exit commit under the same pathspec discipline. One commit per terminal; no separate ledger commit. For anchor-stability purposes a run is **open** while the Terminal state line is blank **or reads stopped awaiting user** — that terminal resumes (the run header holds the posed gate, live until answered), so heading anchors stay held; only **completed** and **abort to brainstorming** release them. A stopped run the user chooses to abandon is closed by writing abort.
