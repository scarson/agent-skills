...
**When the cycle is overkill — recommend the snapshot.** The cycle's added value (per-slice planning, plan-review, cross-slice synthesis) pays off when a single pass *structurally cannot hold the codebase* — a large or recurring surface. For a **small, single-context-holdable scope** (one or two ecosystems, and few enough files that a strong model reads them whole with room left to reason — the bound is what fits *comfortably alongside* the packs, brief, and its own analysis, not the raw window size, and current windows put that well above the ~10k production LOC this guidance was first calibrated against), the full cycle's machinery is largely ceremony — field evidence shows a competent single pass recovers most critical/major findings there, so the planning/review tail is poorly justified. In that case the runner SHOULD **say so and recommend the snapshot [`performance-audit`](../performance-audit/SKILL.md)** (the lanes + synthesis, no plan/review cycle) instead, or run it directly if the user just wants findings. Reserve the full cycle for surfaces that genuinely exceed one context window or that will be re-audited on a schedule. (This extends `whole-repo-scoping.md`'s lightweight-vs-full router up to the cycle entrypoint.)
...
### Non-interactive / autonomous fallback (CI, overnight, headless, operator offline)

If no user is reachable to answer the design decisions and opt-outs, the runner MUST NOT block, silently pick scope, or auto-execute. It degrades to a **written-handoff** protocol instead:

1. **Capture, don't ask — with bounded authority.** Write each Phase-5 design decision and opt-out point as an explicit **`[DECISION]` block** — the question, the runner's **recommended default**, the trade-off, and what it is blocked on — in a decision artifact and inline at the affected plan tasks. Triage by **blast radius**: a finding whose fix is **internal and reversible** (localized to a function/module, no public-contract/schema/dependency/architecture change) is scheduled at its default `FIX` disposition; a finding whose fix would change an **API/response contract, a DB schema/migration, a dependency version, or an architectural seam** is routed to the **flagged-deferred queue awaiting operator sign-off** (a `[DECISION]` block, never scheduled as ready). The runner **never self-authorizes** an irreversible or externally-visible change headless.
2. **Plan as DRAFT.** Proceed to Phase 6 but mark the plan header **`DRAFT — awaiting operator sign-off`**; the `[DECISION]` blocks gate the tasks they touch (default disposition is still FIX-all per Phase 6, but a task behind an unresolved decision is flagged not-ready).
3. **Defer plan-review.** Skip Phase 7 until the decisions are signed off — adversarially reviewing a plan whose key design choices are unresolved is low-value (a structural-only pass is OPTIONAL).
4. **Never auto-execute.** Applying changes to the codebase always requires explicit human sign-off; an autonomous run produces the reviewed *artifacts* (audit + DRAFT plan + decisions), never code changes.
5. **Record the mode.** Note `run_mode: autonomous` (and that the Phase-5/7 human gates were degraded) in the plan header / run metadata, so a reader knows the gates were not exercised.

This makes the cycle usable for CI / scheduled / headless runs without each runner re-inventing the protocol.

---

## Phase 6 — Write fix plan

After user input, the runner MUST invoke [`writing-plans-enhanced`](../writing-plans-enhanced/SKILL.md) to create the implementation plan. That skill owns subagent-proofing, TDD, pitfall review, cross-task conflict minimization, and the Living Document Contract — the cycle MUST NOT duplicate them. (If the harness can't invoke a sibling skill *by name* — e.g. subagent dispatch without the skill registry — read its `SKILL.md` from the plugin install path and follow it directly, exactly as Phase 2 does for `performance-audit`. In **non-interactive mode**, produce the plan as `DRAFT — awaiting operator sign-off` per the Phase-5 fallback.) The runner MUST pass these **performance-specific instructions** to layer on top:
...
Before committing, the runner MUST review the fix plan for subagent-readiness by invoking [`plan-review-cycle`](../plan-review-cycle/SKILL.md). That skill owns the multi-round adversarial review; the cycle MUST NOT duplicate it. (Same name-invocation fallback as Phases 2 and 6: if it can't be invoked by name, read its `SKILL.md` from the install path and follow it. In **non-interactive mode**, **defer** this review until the Phase-5 design decisions are signed off — see the Phase-5 autonomous fallback.) After it completes, the runner SHOULD log plan-quality observations to the project's pattern store (key `plan-review-<slug>`).

---

## Phase 8 — Commit reports

The runner MUST stage and commit all performance audit cycle artifacts (paths use the `performance-audit` skill's artifact base — `docs/` by default, or the project's existing `design/`/`plans/` convention; see that skill's Artifacts note):
...
**Optimize for this one page — it's the product; the rest is provenance.** A large run produces dozens of files and a long minor tail; the roll-up is what the reader acts on. Rank it **value-weighted**, not count-weighted: lead with the handful of high-leverage themes and the few findings that move the needle, and treat the long list of minors as a referenced appendix, not the headline. A run that surfaces 100 findings whose real signal is five themes should make those five themes impossible to miss. (Field evidence: across runs, "the part that actually mattered fit on one page.")
