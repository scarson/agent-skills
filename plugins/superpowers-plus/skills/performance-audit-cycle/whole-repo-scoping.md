...
- **Same axis at two scales:** the one-program-or-many step (the deployable split) and the
  ...
  per-service split. *Worked example: a FastAPI backend + Angular SPA in one repo = **one** process
  boundary → at least one slice per side (e.g. backend-read, backend-ingest, frontend), NOT a
  service-monorepo "one plan per service" split. Reserve the deployable split for an actual multi-service
  repo (several independently-deployed APIs/workers).*
...
- **COLD SWEEP** (one batched run, ~3 lanes: complexity + allocation + data-access)
  ...
  economy is *fewer lanes per run*, not *unbounded LOC per run*. **Early-stop on
  diminishing returns:** a cold sweep that comes back with only minors after its
  first area is *confirming cold*, not under-digging — record "confirmed cold" and
  stop expanding it rather than spending more sweeps to mine nits. (Field evidence:
  a 3-lane sweep over an infra-glue slice returned a single MAJOR; value is
  concentrated in the hot core + the roll-up, not the cold tail's long minor list.)
