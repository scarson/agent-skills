...
`docs/plans/2026-06-03-performance-audit-decisions-log.md`
...
- **The justification test (the anchor): a durable bullet must add what a strong model can't self-supply.**
  It earns its place only as a **grounded specific** (verified against the source), a **version-pinned
  fact** (→ index/brief), or **field-tested judgment** (a non-obvious trade-off, not textbook
  fundamentals). An *ungrounded restatement of what a capable model already knows offers nothing it
  wouldn't do unprompted* — so it is pure cost (bloat, checklist-drag, confident-error risk). Ground it
  or cut it. This is *why* everything load-bearing must be source-grounded (next bullet): grounding is
  what converts a bullet from "stuff the model knows" into "a fact the model can rely on." See
  `generic-pack.md` "The packs are REFERENCES, not checklists".
...
- **Pursue durable accuracy.** A wrong-but-confident bullet is worse than none. New packs/modules are
  written by research agents and then **reviewed for accuracy by the integrator before they ship** — and
  that review must include **source-grounding** every load-bearing or fast-moving specific against
  fetched authoritative docs (not just verify-tagging it). Verify-tags signal uncertainty to the
  consumer; they do not catch an author's confident error — only opening the source does (see
  `generic-pack.md` "How to add a profile pack", step 3, for the lived example that motivated this).
...
- **Core + load-on-detection modules (large ecosystems).** A large pack's core file holds the
  ...
  **`## Variant notes`** in `.NET` (its Modern-vs-Framework split, the original name),
  **`## Compiler, build & ABI notes`** in C++ (which has no managed runtime — its cross-cutting reality
  is the compiler and memory layout), and **`## Reading the plan & schema`** in SQL. Tech-specific depth lives in
...
**Source-ground before shipping — verify every load-bearing claim against the authoritative source, not
training memory (the quality floor).** ("Source-grounding" is the shorthand used throughout this skill and
its decisions log; it means exactly that verification step — primary-source confirmation, not parametric
recall.) Every load-bearing or fast-moving specific in a pack / module / version-index (an exact API name,
a default value, *which layer owns a behavior*, *whether a feature exists at all*, a landing/stable
version) must be **verified against the fetched authoritative source** before it ships — not written from
training knowledge and merely verify-tagged.
The discipline is a tight loop, the same one used for the four Kubernetes modules and the 2026-06 sweep:

1. **Enumerate** every concrete claim in the file (flags, defaults, versions, ownership, feature-existence).
2. **Fetch** the authoritative source for each load-bearing / fast-moving one — official docs, the
   library repo, the language release notes, the relevant JEP / PEP / Swift-evolution / RFC. *A search
   summary is a lead, not a citation* — open the primary source for anything you will assert.
3. **Reconcile**: fix what's wrong, sharpen what's imprecise, and **cut** anything that fails the
   justification test (an ungrounded restatement of fundamentals a strong model already knows). List the
   pages you actually fetched in the file's `## Sources`.

A **verify-tag protects the consumer from staleness; it does not protect against an author's confident
error** — only opening the source does. The two are complementary, not substitutes. This is the floor:
no pack/index claim ships ungrounded.

**Why this is non-negotiable (the evidence).** The 2026-06 grounding sweep (tracker:
`docs/plans/2026-06-05-grounding-sweep-tracker.md`)
source-verified ~63 files. Error density tracked **pack age and authorship method, not ecosystem**: the
oldest hand-authored packs and the drifted version indexes held every genuine error (a non-existent flag
`-fveclibm`; a preview-vs-stable confusion — Scoped Values "stable in Java 23" when it finalized in 25; an
ownership error — DATAS "replaces Server GC" when it works *with* it), while the framework *modules*
across every ecosystem were near-spotless and the two indexes built *with* grounding within days were
already current. The misses were exactly what confident recall gets wrong and only a fetch catches.

**Running a grounding sweep (auditing existing content).** When refreshing the whole library (or after a
long gap), do it as a **tracked, group-by-group pass**, not ad hoc:
- Write a **tracker file** (`docs/plans/<date>-grounding-sweep-tracker.md`) listing every
  pack/module/version-index with a status checkbox, **ordered by API/default density & error-risk**
  (intrinsics/flags and version indexes first; language-agnostic principle files last). The tracker is
  what survives a context reset — keep it current.
- Work **one group per increment**, committing per group with the errors found noted in the message.
- Per file, run the loop above; mark ✅ with a one-line note of what was checked (or the fix applied),
  ➖ for principle-only files with no concrete claims.

**Re-ground the version indexes on a cadence — this is the highest-yield recurring maintenance.** The
durable packs age slowly (idiom-level); the **version indexes are where currency rots** — a
`covered_through` two releases behind silently mis-advises. On each pass: bump `covered_through`, add the
perf-relevant features of newer framework/runtime releases, and **re-verify the most recent version-pins**
(the freshest claims are the least-corroborated, so they're the likeliest to be wrong). Confirm "X is
still experimental / X is now the default / X landed in version N" against the release notes every time —
never carry it forward on memory.
...
- The **design doc**: `docs/plans/2026-06-03-performance-audit-design.md`
...
- `generic-pack.md`: the authoritative pack-authoring guide and the "references, not checklists"
  invariant.
- **The source-grounding floor**: "Source-ground before shipping" in *How to make common changes* above
  is the non-negotiable accuracy procedure (fetch → verify → reconcile → cite); the grounding-sweep
  tracker `docs/plans/2026-06-05-grounding-sweep-tracker.md`
  is a worked example (all ~63 files, the error tally, and the "errors track pack-age not ecosystem"
  finding). Run a sweep when refreshing the library; re-ground the version indexes on a cadence.
...
- [`skill-value-eval-prompt.md`](skill-value-eval-prompt.md) + [`skill-value-meta-analysis-prompt.md`](skill-value-meta-analysis-prompt.md):
  the *measurement* companions to the feedback template — a portable, poolable rubric for "what did the
  skill add over a naïve prompt, and at what repo size does it earn its cost?" (10 scored dimensions, a
  naïve-recovery-% counterfactual, a finding→pack-bullet evidence map), plus a meta-analysis prompt that
  synthesizes N evals into a size-controlled verdict. Hand the eval to a **fresh** agent (the
  author/maintainer is not an independent data point). Field-contributed; see decisions-log Part GG.
