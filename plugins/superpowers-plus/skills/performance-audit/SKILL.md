...
| `pyproject.toml` / `requirements*.txt` / `poetry.lock` | Python; Django/Flask/FastAPI/SQLAlchemy/pandas; data-science stack — numpy/scipy/numba, scikit-learn/xgboost/lightgbm/statsmodels/lifelines, torch/tensorflow/jax, dask/pyspark/ray, pyomo/cvxpy/gurobipy/ortools, geopandas/shapely/rasterio |
| `DESCRIPTION` / `NAMESPACE` / `renv.lock` / `*.Rproj` / sources `*.R`/`*.r`/`*.Rmd` | R — base + data.table / tidyverse (dplyr/tidyr/purrr/readr) / modeling (lm·glm, survival, forecast·fable, caret·tidymodels) |
| `go.mod` | Go + module versions |
| `Cargo.toml` / `Cargo.lock` | Rust + crate versions |
| `CMakeLists.txt` / `Makefile` / `*.vcxproj`/`*.sln` / `meson.build` / Bazel `BUILD` / `conanfile.*` / `vcpkg.json` / `.clang-tidy` / `compile_commands.json`; sources `*.cpp`/`*.cc`/`*.cxx`/`*.hpp`/`*.hh`/`*.h` | C++ — compiler (gcc/clang/msvc) + `-std` level (c++11/14/17/20/23) + stdlib (libstdc++/libc++/MSVC STL) |
...
| CI/CD pipeline config — `.github/workflows/*.yml`, `.gitlab-ci.yml`, `Jenkinsfile`, `.circleci/config.yml`, `azure-pipelines.yml`, `Dockerfile`/`*.dockerfile` | **CI/CD companion pack** (loads *alongside* the language pack) + modules: github-actions, azure-pipelines, gitlab, jenkins, docker-build |
| IaC config — `*.tf`/`*.tofu` + `.terraform.lock.hcl`, `*.bicep`, `cdk.json`, `Pulumi.yaml`, CloudFormation `*.template.yaml`/`.json` (`AWSTemplateFormatVersion`), ARM `azuredeploy.json` | **IaC companion pack** (loads *alongside* the language/CI-CD packs) + modules: terraform, cdk-pulumi, cloudformation-bicep |
| Cloudflare Workers — `wrangler.toml`/`wrangler.jsonc`/`wrangler.json`, binding tables (`[[d1_databases]]`/`[[kv_namespaces]]`/`[[durable_objects]]`/`[[r2_buckets]]`/`[[queues]]`), `@cloudflare/workers-types`, `cloudflare:` imports, `export default { fetch }` | **Cloudflare companion pack** (loads *alongside* the JS/TS or Python/Rust-WASM language pack) + modules: d1, kv, durable-objects |
| Observability instrumentation — OpenTelemetry (`@opentelemetry/*`, `opentelemetry-sdk`, `go.opentelemetry.io/otel`, `OpenTelemetry.*`), metrics clients (`prom-client`, `prometheus_client`, `prometheus/client_golang`, `micrometer`, `prometheus-net`), structured loggers (`structlog`/`pino`/`winston`/`zap`/`zerolog`/`slog`/`tracing`/Logback/Log4j2/Serilog), agent/collector configs (OTel Collector, Grafana Alloy/Agent) | **Observability companion pack** (loads *alongside* the language pack) + modules: opentelemetry, metrics, logging, grafana-alloy |
...
**The CI/CD companion pack** (`cicd.md`) is likewise **content-detected**: load it *alongside* the language pack whenever pipeline/build automation config is material to the scope — GitHub Actions/GitLab/Jenkins/CircleCI/Azure workflows or a `Dockerfile`. It audits **pipeline cost** (wall-time per run, redundant work, caching, job parallelization, image/artifact size) — the *orchestration* that builds/tests/ships the project, not the app's own runtime. Its lanes are the standard slugs *remapped to ops* (`algorithmic`→redundant/repeated work, `data-access`→caching & artifact I/O, `concurrency`→job parallelization, `payload-startup`→image/artifact/cold-start, `idiom-currency`→action/runner currency); its cross-cutting section is **`Pipeline cost model`**. Load `cicd/github-actions.md` when `.github/workflows/` is present and `cicd/docker-build.md` when a `Dockerfile`/BuildKit build is present. The application code the pipeline *runs* stays with the language pack; `cicd.md` is the automation layer.

**The IaC companion pack** (`iac.md`) is likewise **content-detected**: load it *alongside* the language and CI/CD packs whenever Infrastructure-as-Code config is material — Terraform/OpenTofu, AWS CDK, Pulumi, CloudFormation, or Azure Bicep/ARM. It audits the **IaC tooling's operational performance** — `plan`/`apply`/`synth`/`preview` wall-time, state/refresh cost, dependency-graph parallelism, init/synth/asset cost — **not** whether the provisioned cloud architecture is fast (an instance type, a Lambda's memory, a query plan are *out of scope* — a future cloud-runtime pack's job). Its lanes are remapped (`algorithmic`→redundant re-planning, `data-access`→state & provider I/O, `concurrency`→graph parallelism, `payload-startup`→init/synth/asset, `idiom-currency`→provider/tool currency); its cross-cutting section is **`State & dependency-graph model`**. Load `iac/terraform.md` (Terraform/OpenTofu — also the GCP path), `iac/cdk-pulumi.md` (programmatic IaC — also load the language pack), and `iac/cloudformation-bicep.md` (declarative cloud-native) per the signals present.

**The Cloudflare companion pack** (`cloudflare.md`) is **content-detected**: load it *alongside* the language pack (Workers apps are usually JS/TS) whenever a Workers app is in scope — `wrangler.*` config or the binding tables. It is a **runtime/architecture** lens (the complement to IaC's tooling lens): the **Workers execution model** — V8 isolates, the **CPU-time budget** (only CPU counts, not I/O wait), the subrequest limit, statelessness/no connection pooling, Smart Placement, `waitUntil` — plus the latency/consistency trade-offs of the edge data services. Its lanes are remapped (`algorithmic`→CPU budget & request-path compute, `data-access`→edge data access & subrequests, `concurrency`→deferred work, `payload-startup`→bundle & startup, `idiom-currency`→platform-feature currency); its cross-cutting section is **`Workers execution model`**. Load `cloudflare/{d1,kv,durable-objects,r2,queues,cache,hyperdrive}.md` per the bindings present (Workers AI / Vectorize / Workflows stay in the data-access lane until they earn modules).

**The Observability companion pack** (`observability.md`) is **content-detected**: load it *alongside* the language pack whenever telemetry instrumentation (OpenTelemetry, a metrics client, a structured logger, an agent/collector) is material. It audits the **performance cost of the instrumentation itself** — the overhead metrics/logs/traces *add* to the workload — **not** whether the app is adequately observed (that coverage question is the Phase-3 **Measurability** step, not a perf finding). Its lanes are remapped (`algorithmic`→metric cardinality & per-event work, `data-access`→telemetry export I/O, `concurrency`→hot-path blocking/contention, `payload-startup`→SDK/agent init & dep weight, `idiom-currency`→telemetry-stack currency); its cross-cutting section is **`Instrumentation cost model`**. Load `observability/metrics.md` (cardinality), `observability/logging.md` (cross-language structured-logging cost), `observability/opentelemetry.md` (SDK/sampling/export), and `observability/grafana-alloy.md` (the agent/collector tier) per the stack present. It pairs with the Measurability note: that note asks whether you have *enough* telemetry to confirm a fix; this pack asks whether the telemetry you have is *too expensive*.
...
**Sub-stack modules:** if a matched pack carries a `## Framework / sub-stack modules (load on detection)` map (`dotnet.md`, `go.md`, `python.md`, `javascript-typescript.md`, `rust.md`, `cpp.md`, and `r.md` all do), load the **core** pack for the project plus only the `<ecosystem>/<module>.md` files whose detection signals appear in the audit scope (e.g. load `dotnet/sql-server-data.md` only when EF/`SqlClient`/Dapper is present; `go/grpc.md` only when `google.golang.org/grpc`/`.proto` is present; `python/orm-database.md` only when Django ORM/SQLAlchemy/psycopg is present; `python/data-stack.md` when numpy/pandas/polars is central, `python/scientific-computing.md` when scipy/numba/cython numerics are central, `python/machine-learning.md` when scikit-learn/gradient-boosting/forecasting/survival modeling is present, `python/deep-learning.md` when torch/tensorflow/jax is present, `python/distributed-data.md` when dask/pyspark/ray is present, `python/optimization-modeling.md` when pyomo/cvxpy/gurobipy/ortools is present, `python/geospatial.md` when geopandas/shapely/rasterio is present; `javascript-typescript/react.md` only when React/JSX is present; `rust/web.md` only when axum/actix-web/hyper is present, `rust/data-parallelism.md` only when rayon/polars is present; `cpp/numeric-simd.md` only when Eigen/BLAS/SIMD is present, `cpp/networking-async.md` only when Asio/`io_uring`/`epoll` is present). This keeps each run pasting only the relevant tech lenses, not the whole pack. Load a module when its technology is **material to the audit scope**, not on an incidental or transitive import — a lone `import json` / `import asyncio` (Python) or a stray `encoding/json` (Go) that is peripheral to the scoped code does not by itself warrant the serialization or async module; load it when that technology is *central* to the code under audit (the scope is serialization-heavy, or built on asyncio). Detection selects *candidates*; materiality decides the load.
...
The runner MUST dispatch the lanes as **independent, concurrent agents** (embarrassingly parallel — they share no mutable state; packs and the brief are read-only inputs). Read [`lane-prompts.md`](lane-prompts.md) and, for each lane, paste the shared preamble + that lane's body, filling placeholders with the scope, the matched profile-pack slice for that lane — the lane-keyed section of the core pack, **plus the core pack's cross-cutting Runtime/Variant-notes section** (dotnet *Variant notes*; go/python/js-ts/rust *Runtime …notes*; cpp *Compiler, build & ABI notes*; and any companion pack's equivalent — SQL's *Reading the plan & schema*, HTML's *Rendering path & Core Web Vitals*, CI/CD's *Pipeline cost model*, IaC's *State & dependency-graph model*, Cloudflare's *Workers execution model*, Observability's *Instrumentation cost model*), which is shared context that applies to every lane, **plus** any loaded sub-stack modules relevant to the lane, per Phase 0 — the currency brief, and the output file path. Each agent MUST write its raw report to `docs/perf-audits/` **immediately** on completion (persist-before-synthesis) and also return findings for consolidation.
...
- **Lane reads its own slice** (first-class — and the **common** case when lanes are dispatched as subagents that do *not* share the runner's skill registry, so they can't invoke this skill by name). The runner instead tells each lane the exact files to read for itself from this skill's install location — `…/superpowers-plus/skills/performance-audit/profile-packs/<ecosystem>.md` + the relevant `<ecosystem>/<module>.md` + the matching `version-indexes/<ecosystem>.md` — passing only the scope, the brief (or its path), and the output path. (This skill ships in the **`superpowers-plus`** plugin — name it when pointing a subagent at the install path, since a runner who only installed `superpowers` won't have these files.) This avoids the runner holding and re-pasting every pack across 6–8 lanes (a real context cost at scale). Reading-its-own-slice is **not** licence to walk the whole pack as a checklist — the reference-not-checklist rule in the preamble still governs.
...
5. **Measurability note** — note whether the identified hot paths can be *observed* in production: is there existing instrumentation (a metric, a span, a log) that would let someone **confirm the win after the fix**, or would confirming it require adding instrumentation first? For each significant finding, state which — and flag the ones that are currently *unmeasurable* (no signal exists on that path), because "we can't tell if this helped" is itself a finding worth surfacing. Where instrumentation would need to be *added* to measure a fix, weigh that against its cost: instrumentation is not free (the **observability companion pack** audits exactly that overhead — high-cardinality metrics, hot-path logging, synchronous export), so "add a metric here to confirm" should reach for a cheap, bounded signal, not an unbounded label. This is the methodology complement to the observability pack: that pack audits whether your *existing* telemetry is too expensive; this note audits whether you have *enough* of the right telemetry to verify the audit's own findings. (It is **not** a license to recommend telemetry everywhere — flag only the measurement gaps that block confirming a real finding.)
...
> **Artifact base path.** Paths below are written `docs/perf-audits/` and `docs/plans/` by default. If the project has an established docs convention that is *not* `docs/`-rooted (e.g. a `design/` tree, or a top-level `plans/`), the runner SHOULD place these under that convention instead and keep the relative shape (`<base>/perf-audits/…`, `<base>/plans/…`) — match the repo, don't impose `docs/`. State the chosen base once in the consolidated report.
