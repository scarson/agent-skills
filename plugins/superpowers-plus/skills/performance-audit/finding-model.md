...
**Static-run severity is provisional — guard against false authority.** When a run produces **no
`Measured` findings** (no profiler / `EXPLAIN` / benchmark was available — the common autonomous case),
the report MUST say so once, plainly, and treat every severity as an **argued** rank, not a measured
one. The fingerprint + verification-plan + severity scaffolding makes a static argument *look* measured;
do not let it. In particular, a **`CRITICAL` that rests solely on a static argument** (no measurement,
and its real magnitude depends on a runtime quantity you did not observe — cluster depth, row counts,
call frequency) MUST carry an explicit **"provisional — pending measurement"** marker naming the
unobserved quantity. Prefer `MAJOR` + that marker over an unmeasured `CRITICAL` when the gap to
CRITICAL is exactly the thing you couldn't measure. This is not hedging every finding — it is refusing
to let presentation polish convert "structurally bad" into "certainly catastrophic" without evidence.
