...
## Dispatch assembly — fill-in checklist (per lane)

`lane-prompts.md` is the *content*; assembling a runnable subagent prompt is mechanical and repeated
once per lane. To cut that boilerplate, assemble each lane prompt as **shared preamble + lane body**
with these blanks filled once and reused across lanes (only the per-lane lens slice + `<lane>` change):

| Placeholder | Fill with | Same across lanes? |
|---|---|---|
| `[stack profile]` | the Phase-0 `{ecosystem, framework, version}` tuples | yes |
| `[profile-pack lens]` | **either** paste the lane-keyed slice + the cross-cutting Runtime/Variant/Compiler-notes section (+ companion equivalents), **or** (lane-reads-own-pack mode) the *file paths* to read — see below | lens text per-lane; notes shared |
| `[currency brief]` | the brief, its cache path, or `"unavailable — offline"` | yes |
| `[scope]` | the files/area under audit | yes |
| `<date>T<HH-MM>-<slug>-<lane>` | the run's date/time + scope slug + this lane's slug | `<lane>` changes |

**Lane-reads-own-pack variant** (the common case when subagents lack the skill registry — see
`SKILL.md` Phase 2): instead of pasting `[profile-pack lens]`, tell the lane the exact files to read
for itself, e.g. *"Read `…/superpowers-plus/skills/performance-audit/profile-packs/<eco>.md` (your
`<lane>` section + the cross-cutting notes section) + the material `<eco>/<module>.md` + the matching
`version-indexes/<eco>.md`; the reference-not-checklist rule still governs."* Then fill only
`[scope]`, `[currency brief]`, and the output path. Run one such assembled prompt per lane in the Lanes
table (Phase 2), concurrently.
...
Profile-pack lens for your lane: [paste the relevant lane slice from the matched profile pack(s), PLUS the core pack's cross-cutting Runtime/Variant-notes section — dotnet `Variant notes`; go/python/js-ts/rust `Runtime …notes`; cpp `Compiler, build & ABI notes`; and a companion pack's equivalent (SQL `Reading the plan & schema`, HTML `Rendering path & Core Web Vitals`, CI/CD `Pipeline cost model`, IaC `State & dependency-graph model`, Cloudflare `Workers execution model`, Observability `Instrumentation cost model`) — as shared ecosystem context that applies to every lane]
Currency brief (version-specific guidance): [paste brief, or "unavailable — offline"]
Scope: [paste files/area]
Output file: docs/perf-audits/<date>T<HH-MM>-<slug>-<lane>.md
(Other `…-<lane>.md` files you may see in that directory are concurrent SIBLING lanes from THIS same
run — NOT prior runs. Write your own file; do not skip writing, refuse to "overwrite", or treat their
presence as evidence this lane already ran.)
