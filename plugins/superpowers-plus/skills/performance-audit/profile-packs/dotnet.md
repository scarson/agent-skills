...
| **Kubernetes (KubernetesClient / KubeOps)** — the `KubernetesClient` package / `k8s` namespace, `new Kubernetes(config)`, `*Async` list/watch, `SharedInformer`, KubeOps operator code | [`dotnet/kubernetes-client.md`](dotnet/kubernetes-client.md) |
