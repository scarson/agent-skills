...
- **`GOMAXPROCS` unaware of the container CPU limit**: the runtime historically sets `GOMAXPROCS` to
  ...
  explicit `runtime.GOMAXPROCS` set from the cgroup quota) in containerized services; **Go 1.25+ makes the
  runtime container-aware** — it defaults `GOMAXPROCS` to the cgroup CPU limit and re-checks periodically,
  so `automaxprocs` is largely redundant there (still useful on ≤1.24); confirm the toolchain version
  before flagging either way (verify against the currency brief for your version).
...
| **Kubernetes (client-go / controller-runtime)** — `k8s.io/client-go`, `sigs.k8s.io/controller-runtime`, Kubebuilder / Operator-SDK, informers/listers, a `Reconcile(...)` loop | [`go/kubernetes-client.md`](go/kubernetes-client.md) |
