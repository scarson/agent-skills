...
- **`CheckAlways` components inflate re-check scope**: every async event (click, XHR,
  timer, microtask) causes Angular to walk the entire component tree and re-evaluate all template
  expressions in `CheckAlways` components. (`CheckAlways` was the *default* through Angular 21; **v22 makes `OnPush` the default** — see the version index — so on v22+ the finding is a component overridden back to `Eager`/`CheckAlways`.) `OnPush` limits re-checks to: an `@Input` reference
...
- **Zone.js churn from third-party code or frequent microtasks**: Zone.js monkey-patches browser
  ...
  `NgZone.run`. The **zoneless** scheduler (`provideZonelessChangeDetection`; experimental ~18, stabilizing through ~20–22 but still opt-in, not the default) removes
