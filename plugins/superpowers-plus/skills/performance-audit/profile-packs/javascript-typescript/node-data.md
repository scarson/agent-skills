...
- **Serverless connection storms — new pool per invocation without a proxy**: in Lambda/Cloud
  ...
  **Prisma Accelerate** (the connection-pooler successor to the now-retired Prisma Data Proxy) in front; look also for Prisma's default `connection_limit` (which sizes to CPU
