...
- **The GIL governs what concurrency buys you**: a single GIL serializes Python bytecode, so threads
  ...
  free-threaded build (`python3.13t` experimental → **`python3.14t` officially supported** since 3.14,
  PEP 779; single-thread penalty down to ~5–10%) — confirm the interpreter and C-extension `Py_mod_gil`
  readiness before assuming no-GIL (verify against the currency brief for your version).
...
| **Scientific computing** — `scipy` (`sparse`/`optimize`/`signal`/`linalg`/`integrate`/`stats`/`spatial`), `numba`, `cython` | [`python/scientific-computing.md`](python/scientific-computing.md) |
| **Machine learning** — `scikit-learn`/`sklearn`, `xgboost`, `lightgbm`, `catboost`, `statsmodels`, `prophet`, `sktime`, `lifelines`, `scikit-survival`/`sksurv` | [`python/machine-learning.md`](python/machine-learning.md) |
| **Deep learning & GPU** — `torch`/PyTorch, `tensorflow`/`keras`, `jax`/`flax` | [`python/deep-learning.md`](python/deep-learning.md) |
| **Distributed & out-of-core** — `dask`/`dask.distributed`, `pyspark`, `ray` | [`python/distributed-data.md`](python/distributed-data.md) |
| **Scientific/optimization modeling** — `pyomo`, `ortools`/CP-SAT, `cvxpy`, `gurobipy`, `docplex`/`cplex` | [`python/optimization-modeling.md`](python/optimization-modeling.md) |
| **Geospatial** — `geopandas`, `shapely`, `pyogrio`/`fiona`, `rasterio`, `rioxarray` | [`python/geospatial.md`](python/geospatial.md) |
...
| **Kubernetes client** — `kubernetes` / `kubernetes-asyncio`, `client.CoreV1Api`/`AppsV1Api`, `watch.Watch`, KubeSpawner, operator/controller code | [`python/kubernetes-client.md`](python/kubernetes-client.md) |
