...
| **Kubernetes (kube-rs)** — `kube`, `kube-runtime`, `kube-client`, `kube-derive`, a `Controller`/`watcher`/`reflector` | [`rust/kubernetes-client.md`](rust/kubernetes-client.md) |
...
- **Kubernetes (kube-rs)** (`rust/kubernetes-client.md`) — kube-rs optimization guide (reflector `Store`,
  `watcher::Config` selectors, `metadata_watcher`, `streaming_lists`, `predicate_filter`,
  `controller::Config` concurrency/debounce).
