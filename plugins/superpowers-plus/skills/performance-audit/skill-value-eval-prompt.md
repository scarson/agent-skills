...
> **Provenance.** Authored during a real field run and adopted as a maintainer asset (companion to
> [`feedback-template.md`](feedback-template.md)). Where `feedback-template.md` captures the *experience*
> of running the skill, this prompt measures its *marginal value* over a naïve baseline, in a poolable
> shape. Hand it to a **fresh** agent — the skill's author/maintainer is not a valid independent data
> point. Aggregate completed evals with [`skill-value-meta-analysis-prompt.md`](skill-value-meta-analysis-prompt.md).
...
- FINDING VERIFIABILITY (capture this — it bounds D10): what fraction of the run's findings could even
  be *measured* in the available environment (was a DB / profiler / load generator / representative
  dataset present?) vs are unfalsifiable static arguments? A run where nothing could be measured caps how
  much credit "it named the measurement to run" deserves, and is the covariate most likely to explain a
  low D10 or an inflated-looking confidence.
