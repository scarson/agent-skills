...
> **Provenance.** Maintainer asset, companion to [`skill-value-eval-prompt.md`](skill-value-eval-prompt.md).
> Run it once ≥3 independent evals exist (≥6 before trusting any size/score correlation).

**Purpose.** Once several agents have each produced a `skill-value-eval-<repo>-<date>.md` using the
companion prompt ([`skill-value-eval-prompt.md`](skill-value-eval-prompt.md)), hand THIS prompt to
...
(autonomous/HITL, static/dynamic) · **finding verifiability** (measurable vs static-only) · finding
counts · naïve-recovery-% · the ten D-scores. This is the
