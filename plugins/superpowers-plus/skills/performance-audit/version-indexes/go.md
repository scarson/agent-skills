...
- **C-to-Go call overhead reduction** — **Go 1.21** (Unix) — cgo setup preserved across multiple calls from the same thread; cost drops from 1–3 µs to 100–200 ns per call — automatic for existing cgo code; benefits mixed-language hot paths. **Go 1.26** cuts baseline cgo overhead a further ~30%.
- **Container-aware `GOMAXPROCS`** — **Go 1.25** (Linux) — the runtime now reads the cgroup CPU limit and defaults `GOMAXPROCS` to it (and periodically re-checks, adjusting if the limit changes on the fly) — supersedes needing `go.uber.org/automaxprocs` on 1.25+: a CPU-limited container no longer over-provisions Ps and GC-assist, avoiding throttling/tail-latency spikes — automatic on 1.25+; `automaxprocs` becomes largely redundant (still useful on ≤1.24). Verify the runtime version before assuming either behavior.
- **Green Tea GC** — experimental in **Go 1.25** (`GOEXPERIMENT=greenteagc`), **enabled by default in Go 1.26** — memory-aware parallel marking that processes physically-contiguous heap blocks together for better cache locality; ~10% less GC time on many workloads, up to ~40% on GC-heavy ones — automatic on 1.26 (opt-in on 1.25); a workload that GC-thrashes is a candidate to re-measure on 1.26.
- **`goroutineleak` profile** — **Go 1.26** — a built-in profile that detects leaked goroutines using the garbage collector — supersedes ad-hoc leak hunting via `pprof goroutine` diffing; pairs with the core pack's goroutine-leak bullet for confirmation.
...
- **`encoding/json/v2`** — experimental in **Go 1.25** (`GOEXPERIMENT=jsonv2`; package `encoding/json/v2` + `encoding/json/jsontext`) — a rewrite with markedly faster decoding and a streaming-first API, plus stricter/configurable behavior — *do not assume it's the default*: it is opt-in via GOEXPERIMENT as of 1.25/1.26; on hot JSON paths it is the forward path, but until it's the stable default the code-gen/drop-in replacements (`easyjson`/`goccy`) remain the durable hot-path advice. *(verify the milestone for your Go version.)*
...
> Durable client-library idioms (not tied to a Go release), carried so the idiom-currency lane is
> grounded on common backend libraries rather than falling back to model knowledge. Verify the exact API
> against the version in `go.mod`.
>
> **Coverage note:** a finding on a library *not* listed here legitimately carries **LOW/Heuristic**
> confidence pending a live currency brief — that is *expected*, not a research failure.

- **pgx — `pgx.Batch` to pipeline independent statements** — version-independent — sending N independent queries as one `Batch` (`SendBatch`) collapses N network round-trips into one — the dominant win for per-row/per-item DB work; supersedes a loop of `Query`/`Exec` — use whenever the statements don't depend on each other's results.
- **pgx — native `pgx.CollectRows` + `RowToStructByName` over the `database/sql` adapter** — version-independent — the `database/sql` wrapper adds an interface/conversion layer and loses pgx's binary protocol and typed scanning; the native `pgxpool` + `pgx.CollectRows(rows, pgx.RowToStructByName[T])` path avoids per-row `interface{}` boxing and extra copies. Also avoid `QueryExecModeSimpleProtocol` (text protocol, no prepared-statement caching) unless a proxy forces it — supersedes `database/sql` + manual `rows.Scan` on hot read paths.
- **pgx — `CopyFrom` for bulk insert** — version-independent — the COPY protocol is far faster than multi-row INSERT for large batches; supersedes per-row or `INSERT … VALUES (…),(…)` loops when loading many rows.
- **huma / request validation — lean response models, stream large lists** — version-independent — huma validates against the OpenAPI schema per request; keep response models lean (large nested structs re-marshal on every response) and stream large list endpoints rather than materializing the whole slice — verify against the huma version; treat as Heuristic until a brief grounds it.
- **client-go / controller-runtime — read from the cache, index + select** — version-independent — controller-runtime's `mgr.GetClient()` serves `Get`/`List` from the informer cache; keep reads there (use `APIReader` only for guaranteed-live reads), and register field indexers + query with `client.MatchingFields`/`MatchingLabels` rather than filtering in Go — supersedes polling the apiserver and client-side filtering. **Rate-limit nuance:** raw client-go defaults to `QPS=5`/`Burst=10` (raise for hot raw clients — watch for "client-side throttling" logs), but **controller-runtime's `GetConfig` disables client-side limiting by default** and relies on server-side API Priority & Fairness.
- **controller-runtime — predicates + scoped cache** — version-independent — gate `Reconcile` with `predicate.GenerationChangedPredicate{}` and precise `Owns`/`Watches` so your own status writes don't re-trigger the loop (reconcile storms), and scope `cache.Options{ByObject: {selectors, Namespaces}}` + trim `managedFields` so the informer cache doesn't hold the whole cluster — supersedes unfiltered cluster-wide caching and no-op reconciles. *(See `go/kubernetes-client.md`; verify option shapes for your controller-runtime version.)*
