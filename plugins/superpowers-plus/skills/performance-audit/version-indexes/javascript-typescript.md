...
covered_through: "React 19 / Angular 22 / Vue 3.5 (3.6 Vapor in beta) / Node.js 24 LTS"
built_on: 2026-06-05
sources:
  ...
  - https://angular.dev/guide/signals/resource         # resource / rxResource (19→20)
  - https://angular.dev/guide/http/http-resource       # httpResource (19.2+)
  - https://angular.dev/events/v21                      # Angular 21 (stable Nov 2025) — Signal Forms preview
  - https://blog.ninja-squad.com/2026/06/03/what-is-new-angular-22.0  # Angular 22 — OnPush default, resource/httpResource/Signal Forms stable, HttpClient Fetch default, TS6/Node26
...
- **`linkedSignal` / `resource` / `rxResource` API** — `linkedSignal` and `resource` landed experimental in **Angular 19**, evolved through **Angular 20** (the `resource` reactive-source param was renamed `request`→`params`; `rxResource` for an Observable loader), and **graduated to stable in Angular 22** — `linkedSignal` is a writable signal derived from another source; `resource`/`rxResource` manage async data loading with built-in loading/error signals; supersedes manual `computed` + `effect` + `BehaviorSubject` data-loading — *(stable as of v22; on 19–21 treat as experimental and verify the signature for your minor — it changed across 19→20)*.
- **`httpResource()`** — experimental in **Angular 19.2**, continued in **Angular 20**, **stable in Angular 22** — declarative HTTP-as-a-signal built on `HttpClient`: the request is a reactive function, the response is a resource signal with loading/error state; supersedes hand-rolled `HttpClient` + `toSignal`/`BehaviorSubject` wiring for read-and-display data flows — *(stable as of v22; experimental on 19.2–21)*.
- **`OnPush` change detection** — available since Angular 2, and **the default strategy for new components as of Angular 22** (prior versions defaulted to `CheckAlways`; v22's migration tags existing components `ChangeDetectionStrategy.Eager` to preserve old behavior) — a component only re-checks when an `@Input` reference changes, an Observable emits via `async` pipe, or a signal notifies; on ≤v21 it is an opt-in that supersedes `CheckAlways` for data-driven components, on v22+ it is the baseline (so finding it *absent*/overridden to `Eager` is the signal).
- **HttpClient on the Fetch API by default** — **Angular 22** — `HttpClient` now uses `fetch()` under the hood instead of `XMLHttpRequest`; `withFetch()` is deprecated (now the default), and `withXhr()` opts back into the legacy XHR backend — relevant to streaming/response handling and SSR; *(default as of v22; pre-v22 needed `withFetch()` to opt in)*.
- **Signal Forms** — first previewed in **Angular 21**, **stable in Angular 22** — a signal-based forms model (reactivity without the RxJS/`valueChanges` subscription overhead of reactive forms); *(stable as of v22; preview on 21)*.
- **Zoneless change detection (`provideZonelessChangeDetection()`)** — experimental in **Angular 18**, developer preview in **Angular 19**, **stabilizing through Angular 20–22** — removes Zone.js from the dependency graph, eliminating monkey-patching overhead, reducing payload (~14 kB gzip), and improving startup + CD latency; supersedes Zone.js-driven change detection — requires explicit notification via signals, `AsyncPipe`, `markForCheck()`, or reactive forms. *(An app already running zoneless+signals in production is on the modern path; verify the exact GA/stability of `provideZonelessChangeDetection` for your minor against the brief.)*
...
- **Vapor Mode** — **Vue 3.6 (in beta as of this index, not yet GA)** — opt-in compilation strategy that compiles components to direct DOM operations with **no virtual DOM**, reaching Solid/Svelte-class runtime perf without changing component logic; reached feature parity with vDOM mode at 3.6 beta — *do not recommend as GA*: target it only for projects on the 3.6 beta line, otherwise note it as the forward path; the durable advice (narrow reactivity, cache computeds) still applies on stable 3.5. *(verify GA status against the currency brief.)*
...
- **Node.js 24 (current Active LTS, since Oct 2025, "Krypton", supported to ~Apr 2028)** — the LTS line to target as of this index (Node 22 is now maintenance LTS): newer V8 (further Maglev/optimizer gains), **npm 11**, and **stable built-in TypeScript type-stripping** (`--experimental-strip-types` graduated — run `.ts` directly without a transpile step, cutting a build stage for many services). Prefer Node 24 LTS for new deployments; pin upgrade advice to the project's LTS line. *(verify exact feature status against the currency brief for your minor.)*
