...
ecosystem: swift
covered_through: "Swift 6.3 / iOS 26"
built_on: 2026-06-03
sources:
...
- **Swift 6.3 (Mar 2026) — interop/embedded focus, little hot-path change** — **Swift 6.3** — the release is mostly C/C++ interop (`@c` attribute), embedded/systems ergonomics (`@section`/`@used` for memory placement & symbol preservation, Swift MMIO), cross-platform build tooling, and an official Android SDK — *not* a runtime hot-path release. For perf audits, the durable 6.2 items above (Span/InlineArray, nonisolated caller-context, approachable concurrency defaults) remain the substance; treat 6.3 as the current toolchain baseline. *(verify any embedded/interop specifics against the brief.)*
