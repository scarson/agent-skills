...
In this cycle the block's "independent verification rounds" are simply the independent rounds — cold or persistent, same-provider or cross-model alike; self-rounds never qualify, exactly as they never terminate. Every trigger counts those rounds one-for-one — this cycle has no multi-leg closure to group. **Firing-diagnostic measures** (the block's specification-drift diagnostic): size is the plan's line count against the version the cycle's first round reviewed; decision-level content is its task count and per-task verification contracts; concentration is the per-task anchors of the last two rounds' substantive findings.
