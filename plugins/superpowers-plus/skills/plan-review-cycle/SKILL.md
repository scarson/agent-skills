...
The cycle is bounded by independence, not by a round count:
...
- **The cycle cannot end while any rejection is awaiting concurrence.** The terminating round's verdict block must dispose of every handed-over rejection; a missing verdict is a missing output — the runner re-requests it from the reviewer rather than inferring concurrence from silence.
