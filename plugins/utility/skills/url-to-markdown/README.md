...
```
$ scripts/bootstrap.py https://www.anthropic.com/news/model-context-protocol
OK  /your/cwd/2024-11-19-introducing-the-model-context-protocol.md
...
1. **curl_cffi with TLS impersonation** bypasses Cloudflare-class bot fingerprinting that defeats plain `requests`. The skill works on real-world sites like anthropic.com, simonwillison.net, MDN, arxiv.org out of the box.
...
├── examples/
│   ├── mcp-announcement-example.md      # Real sample output (author-less page)
│   └── microsoft-build-2026-example.md  # Real sample output (with author byline)
└── tests/
    ├── fixtures/                     # 6 cached HTML fixtures (anthropic, microsoft, MDN, arxiv, etc.)
...
3. Live-test against all 5 fixture URLs (anthropic, microsoft, MDN, arxiv, simonw) using a real fetch. Ensure the frontmatter still round-trips through PyYAML.
