...
def test_mcp_article_extraction() -> None:
    """anthropic.com — real-world long-form news article, the canonical test case."""
    _md, fm, body = extract(
        "mcp-announcement.html",
        "https://www.anthropic.com/news/model-context-protocol",
    )

    # Frontmatter has the expected metadata fields
    assert "title" in fm and "Model Context Protocol" in fm["title"], \
        f"Expected 'Model Context Protocol' in title, got {fm.get('title')!r}"
    assert fm.get("date", "").startswith("2024-11"), \
        f"Expected date to start with '2024-11', got {fm.get('date')!r}"

    # Body has the article's real content (bounds kept wide to survive
    # trafilatura version drift on a real-world page)
    assert_range(len(body), 3000, 15000, "body length")
    assert_in("open standard", body, "body")
    assert_in("information silos", body, "body")
    assert_in("frontier models", body, "body")

    # Site nav / chrome is stripped (these labels live in the page shell,
    # not the article body)
    assert_not_in("Try Claude", body, "body (nav chrome)")
    assert_not_in("Sign in", body, "body (nav chrome)")

    # Inline markdown links are preserved (include_links=True)
    assert "](" in body, "expected at least one markdown link in body"


def test_microsoft_build_article_extraction() -> None:
    """blogs.microsoft.com — long-form post WITH an author byline and a colon
    in both title and description (a real-world C1 colon-quoting case)."""
    _md, fm, body = extract(
        "microsoft-build-2026.html",
        "https://blogs.microsoft.com/blog/2026/06/02/microsoft-build-2026-be-yourself-at-work/",
    )

    assert "title" in fm and "Microsoft Build 2026" in fm["title"], \
        f"Expected 'Microsoft Build 2026' in title, got {fm.get('title')!r}"
    assert fm.get("author") == "Kyle Daigle", \
        f"Expected author='Kyle Daigle', got {fm.get('author')!r}"
    assert fm.get("date", "").startswith("2026-06"), \
        f"Expected date to start with '2026-06', got {fm.get('date')!r}"

    # Long-form page: keep bounds wide to survive trafilatura version drift
    assert_range(len(body), 5000, 30000, "body length")
    assert_in("developer", body, "body")
    assert_in("Copilot", body, "body")

    # Skip-link / nav chrome is stripped
    assert_not_in("Skip to", body, "body (nav chrome)")
...
def test_ssrf_guard_allows_public_url() -> None:
    ...
    validate_url("https://www.anthropic.com/news/model-context-protocol")
...
def test_mcp_frontmatter_is_valid_yaml() -> None:
    """A real-world page's metadata must round-trip cleanly through PyYAML
    once emitted as frontmatter. Exercises build_frontmatter end-to-end on
    the canonical fixture. (The colon-quoting C1 regression is covered
    directly by test_build_frontmatter_quotes_colon_in_description below,
    which does not depend on any one page happening to contain ': '.)
    """
    import yaml
    import url_to_markdown as utm

    url = "https://www.anthropic.com/news/model-context-protocol"
    html = (FIXTURES / "mcp-announcement.html").read_text(encoding="utf-8")
    body, meta = utm.extract_markdown(html, url=url)
    extras = {
        "source_url": url,
        "fetched": "2024-11-19T00:00:00Z",
        "http_status": 200,
    ...
    try:
        ...
        raise AssertionError(
            f"Frontmatter failed PyYAML round-trip: {e}\n"
            f"Frontmatter text:\n{fm_text}"
        )

    assert parsed is not None, "frontmatter parsed to None"
    title = parsed.get("title") or ""
    assert "Model Context Protocol" in title, f"title field missing or wrong: {title!r}"
    description = parsed.get("description") or ""
    assert "open standard" in description, (
...
def test_build_frontmatter_quotes_colon_in_description() -> None:
    """Regression test for C1: a description containing ': ' must be quoted
    correctly in the emitted frontmatter. Before the fix, an unquoted colon
    in the description produced YAML that PyYAML rejected with ScannerError.

    Self-contained (synthetic metadata) so the regression is exercised
    regardless of which real page is cached as the extraction fixture.
    """
    import types
    import yaml
    import url_to_markdown as utm
    ...
    meta = types.SimpleNamespace(
        title="A Hard Truth: Untested Backups Are a Liability",
        author="Test Author",
        date="2026-04-09",
        url="https://example.com/article/",
        hostname="example.com",
        sitename="Example Press",
        description="Backups create a dangerous illusion: untested recovery is just hope.",
        categories=[],
        tags=[],
        language="en",
    )
    extras = {"source_url": "https://example.com/article/", "http_status": 200}
    fm_text = utm.build_frontmatter(meta, extras)
    # Strip the fence for the parser
    assert fm_text.startswith("---\n") and fm_text.endswith("---\n")
    inner = fm_text[4:fm_text.rfind("\n---\n")]

    try:
        parsed = yaml.safe_load(inner)
    except yaml.YAMLError as e:
        raise AssertionError(
            f"Frontmatter with a colon in title/description failed PyYAML "
            f"round-trip (regression of C1): {e}\nFrontmatter text:\n{inner}"
        )
    assert ":" in (parsed.get("description") or ""), "colon did not survive round-trip"
    assert ":" in (parsed.get("title") or ""), "colon in title did not survive round-trip"
...
def test_all_fixtures_emit_valid_yaml_frontmatter() -> None:
    ...
    across multiple publisher layouts, not just the canonical fixture.
    """
    import yaml
    import url_to_markdown as utm

    fixtures = [
        ("mcp-announcement.html", "https://www.anthropic.com/news/model-context-protocol"),
        ("microsoft-build-2026.html", "https://blogs.microsoft.com/blog/2026/06/02/microsoft-build-2026-be-yourself-at-work/"),
...
def test_extract_generic_trafilatura_returns_result_with_method_name() -> None:
    ...
    assert "Model Context Protocol" in (result.metadata.title or "")
...
def test_run_falls_through_to_generic_for_unknown_host() -> None:
    ...
    result = utm.extract_via_dispatch(
        html, url="https://www.anthropic.com/news/model-context-protocol"
    )
    assert result.extraction_method == "generic_trafilatura"
    assert "Model Context Protocol" in (result.metadata.title or "")
...
def main() -> int:
    ...
    tests: list[Callable[[], None]] = [
        test_mcp_article_extraction,
        test_microsoft_build_article_extraction,
        ...
        test_mcp_frontmatter_is_valid_yaml,
        test_build_frontmatter_quotes_colon_in_description,
